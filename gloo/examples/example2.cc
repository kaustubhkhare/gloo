...
int main(void) {
    ...
    gloo::GatherOptions opts(k_context);
    ...
    auto reduce_scatter =
            std::make_shared<gloo::ReduceScatterHalvingDoubling<int>>(
                    k_context, ptrs, count, recv);

    for (int i = 0; i < 10; i++) {
        reduce_scatter->run();
        opts.setInput(data.data() + rank*inputEle/size, inputEle/size);
        // Run the algorithm.
        gloo::gather(opts);
    }
