...
iters=(500 500 500 500 500 500 500 500 500)
...
for i in `seq 0 8`; do
  echo "Running: PREFIX=${4}-${i}${i} SIZE=${1} RANK=${2} NETWORK=${3} INPUT_SIZE=${cnt[$i]} ITERS=${iters[$i]} $(find . -name $4) &> ~/logs/${4}-${1}.log &" >> ~/logs/${4}-${1}.log
  PREFIX="${4}-${i}${i}" SIZE=$1 RANK=$2 NETWORK=${3} INPUT_SIZE=${cnt[$i]} ITERS=${iters[$i]} $(find . -name $4) &>> ~/logs/${4}-${1}.log
  echo ""
  echo ""
done
