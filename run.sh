...
echo "Running: PREFIX=${4} SIZE=${1} RANK=${2} NETWORK=${netw} INPUT_SIZE=${5} ITERS=${6} $(find . -name $4) &> ~/logs/${4}.log &" > ~/logs/${4}.log
PREFIX=$4 SIZE=$1 RANK=$2 NETWORK=${netw} INPUT_SIZE=$5 ITERS=${6} $(find . -name $4) &>> ~/logs/${4}.log &
